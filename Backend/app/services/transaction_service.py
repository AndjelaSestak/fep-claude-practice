...
def _set_transaction_direction(
    db: Session, transactions: list[Transaction], user_id: int
) -> None:
    """
    ...
    try:
        actual_account = (
            db.query(Wallet.account_number).filter(Wallet.user_id == user_id).scalar()
        )
    except Exception:
        actual_account = None
...
def _build_user_transactions_query(
    ...
    user_account = (
        db.query(Wallet.account_number)
...
def get_transaction_by_user(
    ...
    query = _build_user_transactions_query(
        db, user_id, search=search, type=type, direction=direction
    )
...
def create_transaction(
    db: Session, request: CreateTransactionRequest, current_user: User
) -> Transaction:
    ...
    card = (
        db.query(Card)
        .filter(Card.id == request.card_id, Card.user_id == current_user.id)
        .first()
    )
...
def _process_sender(db: Session, transaction: Transaction) -> bool:
    sender_wallet = (
        db.query(Wallet)
        .filter(Wallet.user_id == transaction.user_id, Wallet.is_active == True)
        .first()
    )
...
def cancel_transaction(
    ...
    try:
        ...
        raise DatabaseTransactionError(
            "An error occurred while cancelling the transaction. Please try again."
        )
