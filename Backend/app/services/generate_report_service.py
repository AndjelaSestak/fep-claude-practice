...
def generate_csv_report(transactions):
    ...
    writer.writerow(["Date", "Recipient", "Amount", "Currency", "Type", "Direction"])
...
def generate_pdf_report(transactions, user_email):
    if not transactions:
        html_content = "<html><body><h1>No transactions available</h1></body></html>"
    ...
    template = env.get_template("report/generate_report_pdf.html")
