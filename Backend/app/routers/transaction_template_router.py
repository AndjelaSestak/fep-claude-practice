...
@router.get(
    "/get_template_details/{template_id}", response_model=TransactionTemplateResponse
)
def get_template_details(
    ...
    return transaction_template_service.get_template_by_id(
        db, template_id, current_user
    )


@router.get("/get_all_templates", response_model=List[TransactionTemplateResponse])
def get_all_templates(
    db: Annotated[Session, Depends(get_db)], current_user: User = Depends(require_user)
):
    return transaction_template_service.get_templates(db, current_user)


@router.patch(
    "/update_template/{template_id}", response_model=TransactionTemplateResponse
)
def update_template(
    template_id: int,
    ...
    return transaction_template_service.update_template(
        db, template_id, request, current_user
    )
...
def execute_template(
    ...
    return transaction_template_service.execute_template(
        db, template_id, current_user, background_tasks, data.pin
    )
